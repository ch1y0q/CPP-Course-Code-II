...
private:
    ...
    friend std::ostream& operator<<(std::ostream& out, HugeInteger obj);
    //friend std::ostream& operator<<(std::ostream& out, HugeInteger obj);
...
HugeInteger::HugeInteger(const char* inStr) {
    ...
    for (int i = (int)initStr.length(); i < 40; i++)
        integer[i] = 0;
}
...
{
    if (minusValue.isGreaterThan(*this)) throw "Unable to deal with negative numbers...";
    for (int i = 0; i < 40; i++) {
        this->integer[i] -= minusValue.integer[i];
        if (this->integer[i] < 0) {
            this->integer[i] += 10;
            this->integer[i + 1]--;
        }
...
void HugeInteger::input(const char* inValue)
{
    ...
    for (int i = (int)setValue.length(); i < 40; i++)
        integer[i] = 0;
}
